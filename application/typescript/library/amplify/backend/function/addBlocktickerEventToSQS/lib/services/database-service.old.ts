...
async function readLastBlockForEventsProcessed() {
  ...
  try {
    var input = {
      Key: {
        id: parseInt(process.env.BLOCK_LOOKUP_TABLE_ID_KEY)  
      },
      TableName: `${process.env.BLOCK_LOOKUP_TABLE_NAME}`,
    };
    console.log('input for GetCommand is', input);

    const command = new GetCommand(input);
    const response = await ddbDocClient.send(command);

    console.log("readLastBlockForEventsProcessed response is", response);
